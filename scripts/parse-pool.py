#!/usr/bin/env python3
"""Parse the 2024-2028 Extra Class question pool text into questions.json."""
...
SUBELEMENTS = {
    "E1": {"name": "Commission's Rules", "exam_questions": 6},
    "E2": {"name": "Operating Procedures", "exam_questions": 5},
    "E3": {"name": "Radio Wave Propagation", "exam_questions": 3},
    "E4": {"name": "Amateur Practices", "exam_questions": 5},
    "E5": {"name": "Electrical Principles", "exam_questions": 4},
    "E6": {"name": "Circuit Components", "exam_questions": 6},
    "E7": {"name": "Practical Circuits", "exam_questions": 8},
    "E8": {"name": "Signals and Emissions", "exam_questions": 4},
    "E9": {"name": "Antennas and Transmission Lines", "exam_questions": 8},
    "E0": {"name": "Safety", "exam_questions": 1},
}

# Questions withdrawn from the pool by errata
DELETED_QUESTIONS = {"E2A13", "E4D05", "E6D07", "E9E10"}

# Regex for question header: e.g. "E1A01 (D) [97.305, 97.307(b)]"
Q_HEADER = re.compile(
    r'^(E[0-9][A-Z]\d{2})\s+\(([A-D])\)\s*(?:\[([^\]]*)\])?\s*$'
)
...
def validate(data: dict) -> list[str]:
    ...
    for q in data['questions']:
        ...
        # ID format check
        if not re.match(r'^E[0-9][A-Z]\d{2}$', q['id']):
...
def main():
    pool_dir = Path(__file__).parent.parent / 'pools' / '2024-2028'
    raw_file = Path('/tmp/extra-pool-raw.txt')
    ...
    for se in sorted(se_counts.keys(), key=lambda x: (x[1] == '0', x)):
